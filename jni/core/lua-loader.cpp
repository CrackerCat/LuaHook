...
static int lua_hook(lua_State* l)
{
	...
	if (!hook_inner((void *)lua_tointeger(l, 1), lua_tostring(l, 2)))
		return 1;
...
static int lua_get_addr(lua_State *l)
{
	...
	void *p_lib = dlopen(luaL_tolstring(l, 1, NULL), RTLD_LAZY);
	void *addr = dlsym(p_lib, luaL_tolstring(l, 2, NULL));
	dlclose(p_lib);
...
static int lua_s2i(lua_State *l)
{
	...
	lua_pushinteger(l, (int)lua_tostring(l, 1));
	return 1;
}
...
static int lua_search_property(lua_State *l)
{
	...
	lua_pushinteger(l, search_prop(lua_tointeger(l, 1), lua_tostring(l, 2)));
	return 1;
}
